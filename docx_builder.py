...
def _append_rows_as_table(doc, current_anchor, headers, rows):
    """
    ...
    # Try to append into the previously injected table for consistent formatting.
    anchor_table = None
    ...
    # Drop duplicates already present in anchor table by first-column key.
    filtered_rows = rows[:]
    if anchor_table is not None:
        existing_first_col = set()
        for r_idx, r in enumerate(anchor_table.rows):
            if r_idx == 0:
                continue
            if not r.cells:
                continue
...
def _inject_docx_content(
    ...
    for idx, element in enumerate(body_elements):
        ...
        if table_only and not is_table:
            continue
        if table_only and keyword and is_table:
            ...
            starts_with_table = bool(
                re.match(r'^table\s*[\.:]?\s*\d*\b', table_text_lower)
                or re.match(r'^table\s*[\.:]?\s*\d*\b', caption_text_lower)
            )
            has_keyword = (
                (keyword in table_text_lower) or
                (keyword in caption_text_lower)
            )
            if not (starts_with_table and has_keyword):
                continue
        try:
    ...
    if table_only and table_count == 0 and not keyword:
        logger.warning(
            f"Section {section_num}: table-only mode enabled but no matching "
            f"tables found (keyword='{table_text_keyword}'). "
            f"Falling back to full content injection."
        )
    ...
    elif table_only and table_count == 0 and keyword:
        logger.warning(
            f"Section {section_num}: no table matched strict rule "
            f"(must start with 'Table' and contain '{table_text_keyword}'). "
